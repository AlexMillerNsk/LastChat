...
type Logger private () =
    ...
    do 
        let directory    = Path.GetDirectoryName(assembly.Location.Replace(@"file:///", String.Empty))
    ...
    static member Error(message: String)     = Logger.Log.Error(message)
    static member Error(message: Exception)  = Logger.Log.Error(message)